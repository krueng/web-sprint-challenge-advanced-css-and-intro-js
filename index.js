...
function getArtistByIndex(arry, arrIdx) {
  /*Your Code Here*/
  return `the artist at index ${arrIdx} is ${arry[arrIdx].name}`;
}
...
//     if (arry[i].years.split('19')[0].length === 0 && arry[i].years.split('19')[2].length === 2) artistsName.push(artists[i].name);
//   }
//   return artistsName;
...
function removeArtist(arry, indx) {
  /*Your Code Here*/
  arry.splice(indx, 1)
  return arry.length;
}
...
function addArtist(arry) {
  ...
  arry.push(
    { 
      ...
      bio: "Javascript is Fun! - Say WHAT! :) " 
    }  
  );
  return arry;
}
...
function lotsOfArt(arry) {
  ...
  const artistsName =[];
  for (let i = 0; i < arry.length; i++) {
    if (arry[i].paintings>100)artistsName.push(arry[i].name)
